...
- 👍 Post reactions (like, love, clap, insightful, laugh)
...
- `GET /api/v1/reactions/?post={id}` - Get post reactions
- `POST /api/v1/reactions/` - Add/update reaction (authenticated)
- `DELETE /api/v1/reactions/{id}/` - Remove reaction (authenticated)
...
- `POST /api/v1/upload-image/` - Upload inline images for post content (authenticated)
...
- Excerpt, meta description, canonical URL (SEO)
...
- Post reference, user, reaction type
- Reaction types: like, love, clap, insightful, laugh
- Unique constraint (one reaction per user per post)
- Timestamp (created_at)

### PostImage

- Post reference (optional for inline images)
- Image file, upload timestamp
...
- ✅ Post reactions (like, love, clap, insightful, laugh)
- ✅ SEO features (slugs, meta descriptions, excerpts)
- ✅ Performance optimizations (database indexing, cached counts)
